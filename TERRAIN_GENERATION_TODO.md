...
- [x] 初期：单线程，小范围生成测试
- [x] 中期：引入 Numba @njit 加速热点函数（60）
- [ ] 后期：实现 Multi-threading 多线程并行（61），每个区块独立生成
- [x] 实施 Heightmap Caching 高度图缓存（62），避免重复计算
- [ ] 实现 LOD 细节分级（63），远处用低分辨率
- [x] 完善 Chunk-based Generation 区块生成（59），按需生成
- [x] 面向感知区块加载优化
...
**进行中**：
- 🔄 喀斯特地貌生成算法实现（karst_generator.py 已创建，尚未集成到地形生成）

**待开始**：
- ⏳ 阶段六：喀斯特地貌与 3D 洞穴（算法实现中，待集成）
- ⏳ 阶段七：冰川、构造与特殊地表
- ⏳ 阶段八：气候、生物群系与方块放置
- ⏳ 阶段九：性能优化与整合
